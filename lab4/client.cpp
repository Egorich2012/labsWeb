...
bool connect_to_server() {
    ...
    strcpy(hello_msg.payload, "Client");
    hello_msg.length = htonl(strlen(hello_msg.payload) + 1);
    ...
    if (auth_response.type == MSG_ERROR) {
        ...
        close(new_sock);
        return false;
    } else if (auth_response.type == MSG_SERVER_INFO) {
...
void* receive_thread(void* arg) {
    while (true) {
        ...
        Message msg;
        int result = receive_message(current_sock, msg);
    ...
    while (!should_exit) {
        if (!connected) {
            if (connect_to_server()) {
                ...
                cout << "\nCommands: /quit, /ping, /w <nick> <message>" << endl;
                cout << "> " << flush;
            }
            else {
        ...
        if (!is_connected || current_sock == -1) {
            continue;
        }
        ...
        if (strcmp(input, "/quit") == 0) {
            ...
            if (space) {
                *space = '\0';
                ...
                snprintf(private_payload, sizeof(private_payload), "%s:%s", target, message);
                send_message(current_sock, MSG_PRIVATE, private_payload);
                cout << "> " << flush;
                *space = ' '; // Восстанавливаем исходную строку
            } else {
                cout << "Usage: /w <nickname> <message>" << endl;
                cout << "> " << flush;
            }
        }
